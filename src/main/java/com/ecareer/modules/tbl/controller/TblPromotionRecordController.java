...
public class TblPromotionRecordController {
    ...
    @ApiOperation(value = "根据当前用户获取晋升记录")
    @GetMapping(value = "/user")
    public CommonResult<List<PromotionRecordVO>> getById(Principal principal) {
        List<PromotionRecordVO> promotionRecordVO = promotionRecordService.getPromotionRecordByUsername(principal.getName());
        return CommonResult.success(promotionRecordVO);
    }

    @ApiOperation(value = "新增当前用户晋升记录")
    @PostMapping(value = "/user")
    public CommonResult<Object> add(@Validated @RequestBody TblPromotionRecordParam promotionRecordParam,
                                    Principal principal) {
        int status = promotionRecordService.addPromotionRecord(principal.getName(), promotionRecordParam);
        if (status > 0) {
    ...
    @ApiOperation(value = "根据当前用户的晋升记录ID删除晋升记录")
    @DeleteMapping(value = "/user/{id}")
    public CommonResult<Object> delete(@PathVariable Long id, Principal principal) {
        int status = promotionRecordService.deletePromotionRecordByUsername(principal.getName(), id);
        if (status > 0) {
            return CommonResult.success();
        } else if (status == -1) {
            return CommonResult.failed("晋升记录不存在");
        } else if (status == -2) {
            return CommonResult.failed("晋升记录不属于该用户");
        } else {
            return CommonResult.failed();
        }
    }
    ...
    public CommonResult<Object> delete(@PathVariable Long id) {
        int status = promotionRecordService.deletePromotionRecordById(id);
        if (status > 0) {
            return CommonResult.success();
        } else if (status == -1) {
            return CommonResult.failed("晋升记录不存在");
